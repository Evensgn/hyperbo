...
if __name__ == '__main__':
    ...
    bins = np.histogram(np.hstack((continuous_lengthscale_list, discrete_lengthscale_list)), bins=30)[1]
    counts, _ = np.histogram(continuous_lengthscale_list, bins=bins)
    plt.stairs(counts, bins, label='Continuous', color='C0', alpha=0.5, fill=True)
    counts, _ = np.histogram(discrete_lengthscale_list, bins=bins)
    plt.stairs(counts, bins, label='Discrete', color='C1', alpha=0.5, fill=True)

    fig.legend()
    plt.tight_layout()
    fig.savefig(os.path.join(results_dir, 'hpob_normalize_xy_only_fitting_matern52', 'lengthscale_distributions.pdf'),
                bbox_inches='tight')
    plt.close(fig)
    '''
