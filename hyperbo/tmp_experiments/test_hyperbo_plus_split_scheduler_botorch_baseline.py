...
if __name__ == '__main__':
    group_id = 'split_hpob_botorch_1' #  'split_hpob_neg_2'
    python_cmd = '/home/azureuser/hyperbo/env-pd/bin/python'
    worker_path = '/home/azureuser/hyperbo/hyperbo/experiments/test_hyperbo_plus_split_worker_botorch_baseline.py'
    ...
    for test_id in test_id_list:
        new_key, key = jax.random.split(key)
        sub_process_i = subprocess.Popen([python_cmd, worker_path, '--group_id', group_id,
                                          '--mode', 'test_bo_setup_a_id_botorch', '--dataset_id', test_id, '--key_0',
                                          str(new_key[0]), '--key_1', str(new_key[1])])
        sub_process_list.append(sub_process_i)
    for sub_process_i in sub_process_list:
        sub_process_i.wait()
    ...
    for test_id in setup_b_id_list:
        new_key, key = jax.random.split(key)
        sub_process_i = subprocess.Popen([python_cmd, worker_path, '--group_id', group_id,
                                          '--mode', 'test_bo_setup_b_id_botorch', '--dataset_id', test_id, '--key_0',
                                          str(new_key[0]), '--key_1', str(new_key[1])])
        sub_process_list.append(sub_process_i)
    for sub_process_i in sub_process_list:
        sub_process_i.wait()
    ...
    new_key, key = jax.random.split(key)
    sub_process_0 = subprocess.Popen(
        [python_cmd, worker_path, '--group_id', group_id, '--mode', 'merge_botorch',
         '--dataset_id', '', '--key_0', str(new_key[0]), '--key_1', str(new_key[1])])
    sub_process_0.wait()
    time_10 = time.time()
