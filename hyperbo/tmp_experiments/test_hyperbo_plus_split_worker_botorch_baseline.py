...
def run_bo_botorch(run_args):
    (key, cov_func, n_dim, queried_sub_dataset, ac_func, init_index, budget) = run_args
    ...
    botorch_observations, _, _ = bayesopt.run_bo_with_botorch(
        dataset=dataset,
        sub_dataset_key=sub_dataset_key,
        queried_sub_dataset=queried_sub_dataset,
        ac_func_type=ac_func,
        iters=budget,
        init_random_key=key
    )
    ...
    botorch_regrets = []
    max_y = -jnp.inf
    for y in botorch_observations[1]:
        if y[0] > max_y:
            max_y = y[0]
        botorch_regrets.append((max_f - max_y) / (max_f - min_f))

    gc.collect()
    print('run bo done')
    return botorch_regrets


def test_bo_botorch(key, pool, dataset, cov_func, n_init_obs, init_index, budget, n_bo_runs, n_bo_gamma_samples, ac_func, bo_sub_sample_batch_size):
    ...
    for i in range(n_bo_runs):
        for sub_dataset_key, sub_dataset in dataset.items():
            ...
            q += 1
            new_key, key = jax.random.split(key)
            task_list.append((new_key, cov_func, n_dim, sub_dataset, ac_func, init_index_i, budget))
    ...
    for task in task_list:
        ...
        task_outputs.append(run_bo_botorch(task))
    '''
    task_outputs = pool.map(run_bo_botorch, task_list)
    '''

    print('task_outputs computed')
    n_sub_datasets = len(dataset)

    botorch_regrets_list = []
    for i in range(n_bo_runs):
        botorch_regrets_i_list = []
        for task_output in task_outputs[i * n_sub_datasets: (i + 1) * n_sub_datasets]:
            botorch_regrets = task_output
            botorch_regrets_i_list.append(botorch_regrets)
        botorch_regrets_list.append(jnp.mean(jnp.array(botorch_regrets_i_list), axis=0))
    botorch_regrets_list = jnp.array(botorch_regrets_list)
    botorch_regrets_mean = jnp.mean(botorch_regrets_list, axis=0)
    botorch_regrets_std = jnp.std(botorch_regrets_list, axis=0)
    return botorch_regrets_mean, botorch_regrets_std, botorch_regrets_list
...
def split_test_bo_setup_a_id_botorch(dir_path, key, test_id, dataset_func_combined, cov_func, n_init_obs, budget, n_bo_runs,
    ...
    for ac_func_type in ac_func_type_list:
        ...
        new_key, key = jax.random.split(key)
        botorch_regrets_mean, botorch_regrets_std, botorch_regrets_list = \
            test_bo_botorch(new_key, pool, dataset, cov_func, n_init_obs, None, budget, n_bo_runs, n_bo_gamma_samples, ac_func, bo_sub_sample_batch_size)
        results[ac_func_type] = {
            'botorch_regrets_mean': botorch_regrets_mean,
            'botorch_regrets_std': botorch_regrets_std,
            'botorch_regrets_list': botorch_regrets_list
        }
    np.save(os.path.join(dir_path, 'split_test_bo_setup_a_id_botorch_prior_{}.npy'.format(test_id)), results)
...
def split_test_bo_setup_b_id_botorch(dir_path, key, test_id, dataset_func_split, cov_func,
    ...
    for ac_func_type in ac_func_type_list:
        ...
        new_key, key = jax.random.split(key)
        botorch_regrets_mean, botorch_regrets_std, botorch_regrets_list = \
            test_bo_botorch(new_key, pool, dataset, cov_func, n_init_obs, init_index, budget, n_bo_runs, n_bo_gamma_samples, ac_func,
    ...
    np.save(os.path.join(dir_path, 'split_test_bo_setup_b_id_botorch_{}.npy'.format(test_id)), results)
...
def split_merge_botorch(dir_path, key, group_id, extra_info, train_id_list, test_id_list,
    ...
    for ac_func_type in ac_func_type_list:
        ...
        botorch_regrets_all_list = []

        for test_id in test_id_list:
            results_a['bo_results'][ac_func_type][test_id] = np.load(os.path.join(dir_path, 'split_test_bo_setup_a_id_botorch_prior_{}.npy'.format(test_id)), allow_pickle=True).item()[ac_func_type]

        for i in range(n_bo_runs):
            botorch_regrets_all_list_i = []

            for test_id in test_id_list:
                botorch_regrets_ij = results_a['bo_results'][ac_func_type][test_id]['botorch_regrets_list'][i]

                botorch_regrets_all_list_i.append(botorch_regrets_ij)

            botorch_regrets_all_list.append(jnp.mean(jnp.array(botorch_regrets_all_list_i), axis=0))

        botorch_regrets_all_list = jnp.array(botorch_regrets_all_list)
        botorch_regrets_mean_total = jnp.mean(botorch_regrets_all_list, axis=0)
        botorch_regrets_std_total = jnp.std(botorch_regrets_all_list, axis=0)

        results_a['bo_results_total'][ac_func_type] = {
            'botorch_regrets_all_list': botorch_regrets_all_list,
            'botorch_regrets_mean': botorch_regrets_mean_total,
            'botorch_regrets_std': botorch_regrets_std_total
        }
    ...
    for ac_func_type in ac_func_type_list:
        ...
        for test_id in setup_b_id_list:
            results_b['bo_results'][ac_func_type][test_id] = np.load(os.path.join(dir_path, 'split_test_bo_setup_b_id_botorch_{}.npy'.format(test_id)), allow_pickle=True).item()[ac_func_type]

        for i in range(n_bo_runs):
            botorch_regrets_all_list_i = []

            for test_id in setup_b_id_list:
                botorch_regrets_ij = results_b['bo_results'][ac_func_type][test_id]['botorch_regrets_list'][i]
    ...
    with open(os.path.join(dir_path, 'results.txt'), 'w') as f:
        ...
        for ac_func_type in ac_func_type_list:
            ...
            for test_id in test_id_list:
                f.write('test_id = {}\n'.format(test_id))
                f.write('botorch_regrets_mean = {}\n'.format(
                    results_a['bo_results'][ac_func_type][test_id]['botorch_regrets_mean']))
                f.write('botorch_regrets_std = {}\n'.format(
                    results_a['bo_results'][ac_func_type][test_id]['botorch_regrets_std']))
                f.write('\n')
            f.write('botorch_regrets_mean_total = {}\n'.format(
                results_a['bo_results_total'][ac_func_type]['botorch_regrets_mean']))
            f.write('botorch_regrets_std_total = {}\n'.format(
                results_a['bo_results_total'][ac_func_type]['botorch_regrets_std']))
            f.write('\n')
        ...
        for ac_func_type in ac_func_type_list:
            ...
            for test_id in setup_b_id_list:
                f.write('test_id = {}\n'.format(test_id))
                f.write('botorch_regrets_mean = {}\n'.format(
                    results_b['bo_results'][ac_func_type][test_id]['botorch_regrets_mean']))
                f.write('botorch_regrets_std = {}\n'.format(
                    results_b['bo_results'][ac_func_type][test_id]['botorch_regrets_std']))
                f.write('\n')
            f.write('botorch_regrets_mean_total = {}\n'.format(
                results_b['bo_results_total'][ac_func_type]['botorch_regrets_mean']))
            f.write('botorch_regrets_std_total = {}\n'.format(
                results_b['bo_results_total'][ac_func_type]['botorch_regrets_std']))
            f.write('\n')
...
if __name__ == '__main__':
    ...
    if args.mode == 'test_bo_setup_a_id_botorch':
        split_test_bo_setup_a_id_botorch(dir_path, key, args.dataset_id, dataset_func_combined, cov_func, n_init_obs, budget, n_bo_runs,
                                 n_bo_gamma_samples, ac_func_type_list,
                                 gt_gp_distribution_params, bo_sub_sample_batch_size)
    elif args.mode == 'test_bo_setup_b_id_botorch':
        split_test_bo_setup_b_id_botorch(dir_path, key, args.dataset_id, dataset_func_split, cov_func,
                                 n_init_obs, budget, n_bo_runs, n_bo_gamma_samples, ac_func_type_list,
                                 gt_gp_distribution_params, bo_sub_sample_batch_size)
    elif args.mode == 'merge_botorch':
        split_merge_botorch(dir_path, key, args.group_id, extra_info, train_id_list, test_id_list,
                    setup_b_id_list, n_workers, kernel_name, cov_func, objective, opt_method, budget, n_bo_runs,
